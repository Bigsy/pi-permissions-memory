...
export type PermissionGateResult =
  | {
      ...
      globalApproval?: { surface: string; pattern: string };
    }
    ...
    if (decision.state === "approved_for_session" && params.sessionApproval) {
      return {
        ...
        sessionApproval: {
          ...
          pattern: decision.approvalPattern ?? params.sessionApproval.pattern,
        },
      };
    }
    if (decision.state === "approved_globally" && params.sessionApproval) {
      return {
        action: "allow",
        globalApproval: {
          ...params.sessionApproval,
