...
export interface SessionApprovalSuggestion {
  ...
  label: string;
  /** Human-readable label for the global approval dialog option. */
  globalLabel: string;
}
 ...
 * - Arity prefix covers all tokens: trailing wildcard (`make build*`).
...
function buildLabel(
  ...
  const suffix =
    scope === "session" ? "for this session" : "always (global config)";
  switch (surface) {
    case "bash":
      return `Yes, allow bash "${pattern}" ${suffix}`;
    case "mcp":
      return `Yes, allow mcp tool "${pattern}" ${suffix}`;
    case "skill":
      return `Yes, allow skill "${pattern}" ${suffix}`;
    case "external_directory":
      return `Yes, allow access to external directory "${pattern}" ${suffix}`;
    default:
      // Tool surfaces (read, write, edit, grep, find, ls, extension tools)
      return `Yes, allow tool "${surface}" ${suffix}`;
  }
}
...
export function suggestSessionPattern(
  ...
  return {
    surface,
    pattern,
    label: buildLabel(pattern, surface, "session"),
    globalLabel: buildLabel(pattern, surface, "global"),
  };
}
