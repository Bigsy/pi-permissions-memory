...
export async function runGateCheck(
  ...
  const hasGlobalApproval =
    gateResult.action === "allow" && gateResult.globalApproval !== undefined;
  ...
  // 6. Record session/global approval(s)
  if (gateResult.action === "allow" && gateResult.sessionApproval) {
    const sessionApproval = gateResult.sessionApproval;
    if (
      descriptor.sessionApproval &&
      "patterns" in descriptor.sessionApproval &&
      sessionApproval.pattern === descriptor.sessionApproval.patterns[0]
    ) {
      ...
      deps.approveSessionRule(sessionApproval.surface, sessionApproval.pattern);
    }
  }

  if (gateResult.action === "allow" && gateResult.globalApproval) {
    deps.approveGlobalRule(
      gateResult.globalApproval.surface,
      gateResult.globalApproval.pattern,
    );
  }
