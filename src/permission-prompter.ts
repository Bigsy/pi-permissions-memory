...
export interface PromptPermissionDetails {
  ...
  sessionLabel?: string;
  /** Override label for the global approval dialog option. */
  globalLabel?: string;
  /** Wildcard pattern backing session/global approvals. */
...
export class PermissionPrompter implements PermissionPrompterApi {
  ...
  async prompt(
    ...
    const decision = await confirmPermission(
      ...
      details.approvalPattern
        ? {
            sessionLabel: details.sessionLabel,
            globalLabel: details.globalLabel,
            approvalPattern: details.approvalPattern,
          }
